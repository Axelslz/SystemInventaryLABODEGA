...
const Navbar = () => {
      ...
      <List>
          ...
          return (
            <ListItem key={item.text} disablePadding>
              <ListItemButton component={RouterLink} to={item.path} onClick={handleDrawerToggle} selected={isActive}
                sx={{ '&.Mui-selected': { bgcolor: mode === 'dark' ? 'rgba(21, 101, 192, 0.3)' : 'rgba(21, 101, 192, 0.12)', borderRight: '4px solid #1565C0' } }}
              >
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={toggleTheme}>
            <ListItemIcon sx={{ color: mode === 'dark' ? '#ffd54f' : 'inherit' }}>{mode === 'dark' ? <Brightness7 /> : <Brightness4 />}</ListItemIcon>
  ...
  return (
    <>
      <AppBar position="fixed" elevation={4} sx={{ background: mode === 'dark' ? '#121212' : 'linear-gradient(90deg, #1565C0 0%, #0D47A1 100%)', color: 'white', top: 0, left: 0, right: 0, width: '100%', zIndex: 1200, borderBottom: mode === 'dark' ? '1px solid #333' : 'none' }}>
        <Container maxWidth="xl">
          <Toolbar disableGutters sx={{ minHeight: '60px' }}> 
            ...
            <Box component={RouterLink} to="/" sx={{ display: 'flex', alignItems: 'center', textDecoration: 'none', position: 'relative', width: { xs: '100px', md: '140px' }, height: '50px', mr: 2 }}>
              <Box component="img" src={logoImg} alt="Logo" sx={{ position: 'absolute', height: { xs: '90px', md: '130px' }, width: 'auto', top: { xs: -20, md: -35 }, left: { xs: -10, md: -40 }, filter: 'drop-shadow(0px 4px 6px rgba(0,0,0,0.4))', zIndex: 1300, transition: 'transform 0.3s ease', '&:hover': { transform: 'scale(1.05)' } }} />
            </Box>
              ...
              {menuItems.map((item) => {
                  ...
                  return (
                    <Box key={item.text}>
                      <Button onClick={handleExpensesClick} startIcon={item.icon} endIcon={<ArrowDropDownIcon />} 
                        sx={{ my: 2, color: 'white', display: 'flex', textTransform: 'none', fontWeight: isExpensesActive ? 'bold' : 'normal', backgroundColor: isExpensesActive ? 'rgba(255, 255, 255, 0.15)' : 'transparent', borderRadius: '8px', px: 2, borderBottom: isExpensesActive ? '3px solid #64b5f6' : '3px solid transparent', '&:hover': { backgroundColor: 'rgba(255, 255, 255, 0.25)', transform: 'translateY(-2px)' } }}
                      >
                        {item.text}
                      </Button>
                ...
                return (
                  <Button key={item.text} component={RouterLink} to={item.path} startIcon={item.icon}
                    sx={{ my: 2, color: 'white', display: 'flex', textTransform: 'none', fontWeight: isActive ? 'bold' : 'normal', backgroundColor: isActive ? 'rgba(255, 255, 255, 0.15)' : 'transparent', borderRadius: '8px', px: 2, borderBottom: isActive ? '3px solid #64b5f6' : '3px solid transparent', transition: 'all 0.3s ease', '&:hover': { backgroundColor: 'rgba(255, 255, 255, 0.25)', transform: 'translateY(-2px)' } }}
                  >
                    {item.text}
                  </Button>
                );
            ...
            <Box sx={{ flexGrow: 0, display: 'flex', alignItems: 'center', gap: 1 }}>
              <IconButton onClick={toggleTheme} color="inherit" sx={{ mr: 1, display: { xs: 'none', md: 'flex' } }}>
                {mode === 'dark' ? <Brightness7 sx={{ color: '#ffd54f' }} /> : <Brightness4 />}
              </IconButton>

              <Box sx={{ textAlign: 'right', mr: 1, display: { xs: 'none', sm: 'block' } }}>
                <Typography variant="body2" fontWeight="bold" sx={{ lineHeight: 1, textTransform: 'uppercase' }}>{user?.username || 'Usuario'}</Typography>
              ...
              <Tooltip title="Opciones">
                <IconButton onClick={handleProfileClick} sx={{ p: 0 }}>
                  <Avatar sx={{ bgcolor: isAdmin ? '#1565C0' : '#ff9800', color: '#fff' }}>
