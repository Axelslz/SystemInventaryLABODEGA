...
import { 
  ...
  FormControlLabel, FormControl, FormLabel, Switch, Slider, Chip,
...
// IMPORTAMOS NUESTRO NUEVO SERVICIO DE USUARIOS
...
const UsuariosTab = () => {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      if (editingUser) {
        ...
        // Creando nuevo
        if (!formData.password.trim()) return showToast('La contraseña es obligatoria para nuevos usuarios', 'warning');
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Estás seguro de eliminar este usuario del sistema? No podrá volver a iniciar sesión.')) return;
    try {
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Box>
            ...
            <Typography variant="body2" color="text.secondary">
                Controla los accesos al sistema. Los usuarios registrados aparecerán como vendedores en el Punto de Venta.
            </Typography>
        </Box>
        <Button 
          variant="contained" 
              ...
              <React.Fragment key={user.id}>
                <ListItem>
                  ...
                  <ListItemSecondaryAction>
                    <IconButton edge="end" aria-label="edit" color="primary" onClick={() => handleOpenEdit(user)} sx={{ mr: 1 }}>
                      <EditIcon />
                    </IconButton>
                    <IconButton edge="end" aria-label="delete" color="error" onClick={() => handleDelete(user.id)}>
                      <DeleteIcon />
                    </IconButton>
                  </ListItemSecondaryAction>
                </ListItem>
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers>
            <Grid container spacing={2}>
                <Grid item xs={12}>
                    <TextField 
                        label="Nombre de Usuario" fullWidth size="small"
                ...
                <Grid item xs={12}>
                    <TextField 
                        label="Contraseña" fullWidth size="small" type="password"
                        value={formData.password}
                        onChange={e => setFormData({...formData, password: e.target.value})}
                        helperText={editingUser ? "Déjalo en blanco para no cambiarla" : "Mínimo 6 caracteres"}
                    />
                </Grid>
                <Grid item xs={12}>
                    <TextField 
                        select label="Rol del Usuario" fullWidth size="small"
                        value={formData.role}
                        onChange={e => setFormData({...formData, role: e.target.value})}
                    >
                        <MenuItem value="empleado">EMPLEADO (Ventas e Inventario)</MenuItem>
...
const TicketTab = () => {
  ...
  <Box sx={{ p: 3 }}>
    ...
    <Grid container spacing={3}>
      <Grid item xs={12} sm={6}><TextField label="Nombre de la Tienda" name="storeName" fullWidth variant="outlined" value={form.storeName} onChange={handleChange}/></Grid>
      <Grid item xs={12} sm={6}><TextField label="Frase o Slogan" name="slogan" fullWidth variant="outlined" value={form.slogan} onChange={handleChange}/></Grid>
      ...
      <Grid item xs={12} sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
        <Button variant="contained" size="large" startIcon={<SaveIcon />} onClick={handleSaveConfig} sx={{ px: 4, borderRadius: 2 }}>Guardar Configuración</Button>
      </Grid>
    </Grid>
  </Box>
  );
...
const PrinterTab = () => {
  ...
  const handleTestPrint = () => alert("Enviando comando de impresión de prueba...");
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={4}>
          ...
          <Paper variant="outlined" sx={{ p: 2, height: '100%' }}>
            <FormControl>
              ...
              <RadioGroup value={paperSize} onChange={(e) => setPaperSize(e.target.value)}>
                <FormControlLabel value="80" control={<Radio />} label={<Typography>80mm (Estándar)</Typography>} sx={{ mb: 1 }}/>
                <FormControlLabel value="58" control={<Radio />} label={<Typography>58mm (Pequeño)</Typography>} />
              </RadioGroup>
            </FormControl>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Paper variant="outlined" sx={{ p: 2, height: '100%' }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 2, color: 'text.secondary' }}>PREFERENCIAS</Typography>
            <List disablePadding>
              <ListItem disableGutters>
                <ListItemText primary="Impresión Automática" />
                <Switch edge="end" checked={autoPrint} onChange={(e) => setAutoPrint(e.target.checked)} />
              </ListItem>
              <Divider component="li" />
              <ListItem disableGutters>
                <ListItemText primary="Mostrar Logo en Ticket" />
                <Switch edge="end" checked={showLogo} onChange={(e) => setShowLogo(e.target.checked)} />
              </ListItem>
            </List>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} sx={{ mt: 2 }}>
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
            <Button variant="outlined" startIcon={<PrintIcon />} onClick={handleTestPrint}>Prueba de Impresión</Button>
            <Button variant="contained" startIcon={<SaveIcon />} size="large">Guardar Configuración</Button>
          </Box>
        </Grid>
      </Grid>
    </Box>
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        <Tabs value={tabIndex} onChange={(e, val) => setTabIndex(val)} indicatorColor="primary" textColor="primary" variant="scrollable">
